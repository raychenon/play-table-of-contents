...
object TableOfContentHelper {
  ...
  private def generateTableOfContent(list: Seq[LineIndex]): Seq[String] = {
    ...
    for (line <- list) yield (s"""${repeatChar(SPACE, line.indentation - min)}${line.title}""")
  }

  private def repeatChar(char: String, n: Int): String = (for (i <- 1 to n) yield char).mkString

  private def minIndentation(l1: LineIndex, l2: LineIndex): LineIndex =
    if (l1.indentation < l2.indentation) l1 else l2
